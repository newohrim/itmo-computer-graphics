...
void DirectionalLight::UpdateBuffer(DeferredRenderer::LightingPass::CBPS& buffer) const
{
...
void PointLight::UpdateBuffer(DeferredRenderer::LightingPass::CBPS& buffer) const
{
	if (buffer.spotLightsNum >= DefaultMeshMaterial::NR_POINT_LIGHTS) {
		return;
	}

	DeferredRenderer::LightingPass::CBPS::PointLight& pointL = buffer.pointLights[buffer.spotLightsNum];
