...
#include "render/DeferredRendering.h" // TODO: TEMP G1
...
void MeshComponent::Draw(Renderer* renderer, bool geomOnly)
{
	...
	if (geomOnly) {
		...
		ShadowMapper::SetCBVS(context, 0, &cbVS);
...
void MeshComponent::DrawGeometry(Renderer* renderer, Shader& geomShader)
{
	auto material = GetMaterial().lock();
	auto context = renderer->GetDeviceContext();
	auto window = renderer->GetWindow();
	...
	DeferredRenderer::GeometryPass::CBPS cbPS;
	cbPS.color = color;
	ThirdPersonCamera* cam = static_cast<ThirdPersonCamera*>(GetGame()->GetActiveCamera());
	cbPS.uCameraPos = Math::Vector4(cam->GetCameraPos());
	cbPS.uSpecPower = 0.0f;
	...
	geomShader.SetCBPS(context, 0, &cbPS);
	DrawComponent::Draw(renderer, true);
}
